...
export class BoardPage extends Component<void> {
  constructor(props) {
    super(props);
    this.state = {
      searchText: "",
  ...
  componentDidMount() {
    ...
    NavAction.subscriptionPageResult(this.props.dispatch, "NewCar", data => {
      if (data.refreshType === true) {
        this.props.Init(1, this.state.searchText, this.props.auth.token);
      }
    });
    NavAction.subscriptionPageResult(this.props.dispatch, "OldCar", data => {
      if (data.refreshType === true) {
        this.props.Init(1, this.state.searchText, this.props.auth.token);
      }
    });
    ...
    this.setState(
      ...
      () => {
        ...
        that.timer = setTimeout(() => {
          ...
          this.bordList.scrollToIndex({ viewPosition: 0, index: 0 });
        }, 500);
      }
    );
  }
  ...
  searchSubmit(event) {
    if (Platform.OS === "android") {
      NativeModules.MyMtaModule.MtaEvent("board_query", "");
    }
  ...
  toReceivecar(value) {
    if (value.receptionStatus !== -1) {
      //isNewClient 1表示新车，0 表示老车
      if (value.isNewClient === 1) {
        if (Platform.OS === "android") {
          NativeModules.MyMtaModule.MtaEvent("board_newCar", "");
        }
        ...
        if (Platform.OS === "android") {
          NativeModules.MyMtaModule.MtaEvent("board_oldCar", "");
        }
        this.props.navigation.navigate("OldCar", {
          license: value.license,
          receptionId: value.receptionId,
          flagFrom: "main"
        });
  ...
  toVcform(value) {
    if (Platform.OS === "android") {
      NativeModules.MyMtaModule.MtaEvent("board_vcForm", "");
    }
  ...
  toConstruction(value) {
    if (Platform.OS === "android") {
      NativeModules.MyMtaModule.MtaEvent("board_construction", "");
    }
  ...
  endSubmit() {
    if (Platform.OS === "android") {
      NativeModules.MyMtaModule.MtaEvent("board_end", "");
    }
        ...
        <View style={styles.jumpsWrap}>
          ...
          >
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                if (Platform.OS === "android") {
                  NativeModules.MyMtaModule.MtaEvent("board_to_carDetail", "");
                }
      ...
      <View style={styles.container}>
        ...
        {this.state.carBrandData.length === 0 ? (
          <View style={styles.emptyImageCon}>
            ...
            <View style={styles.mainListWrap}>
              <FlatList
                ref={bordList => (this.bordList = bordList)}
